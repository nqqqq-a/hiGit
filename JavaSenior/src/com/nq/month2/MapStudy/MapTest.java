 ...
 *                         原因:在原有的HashMap底层结构基础上，添加了一对指针，指向前一个和后一个指针。
 *                         对于频繁的遍历操作，此类执行效率高于HashMap。
 *      |----TreeMap:保证按照添加的key-value对进行排序，实现排序遍历。此时考虑key的自然排序或定制排序
 *                   底层使用红黑树
 *      |---Hashtable:作为Map的古老的实现类；线程安全的，效率低；不能存储null的key和value
 *          |----Properties:常用处理配置文件。key和value都是String类型。
 *
 ...
 *      如果此位置上的数据不为空，(意味着此位置上存在一个或多个数据(以链表形式存在))，比较当前key1和已经存在的数据
 *      的哈希值：
 *              如果key1的哈希值与已经存在的数据(key2-value2)的哈希值都不相同，此时key1-value1添加成功。---情况2
 *              如果key1的哈希值和某一个数据的哈希值相同：此时调用key1所在类的equals(key2)：
 *                                  若equals()返回false，则key1-value1添加成功。---情况3
 *                                  若equals()返回true，则使用value1替换value2。
 *
 *      补充：关于情况2和情况3：此时key1-value1和原来的数据以链表的方式存储。
 *
 *      在不断的添加过程中，会涉及到扩容问题，默认的扩容方式：扩容为原来容量的2倍，并将一样的数据复制过来。
 *
 *      jdk 8相较于jdk 7底层实现方面的不同：
 ...
 *  在Entry的原基础上多建立了 before和after指针。 能够记录添加的元素的先后顺序
 *
 *  五、Map中定义的方法
 添加、删除、修改操作：
 ...
 int size():返回当前map中的key-value对的个数
 boolean isEmpty():判断当前map是否为空
 boolean equals(Object obj):判断当前map和参数对象obj是否相等

 元视图操作的方法：
 Set keySet():返回当前key构成的Set集合
...
//      Object get(Object key):获取指定key对应的value值,不存在则返回null
...
//      boolean isEmpty():判断当前map是否为空
        System.out.println(map.isEmpty()); //判断map中的数据是否存在，并不是简单的map == null
    ...
    Object put(Object key,Object value):将指定key-value添加(或修改)到当前map对象中
    void putAll(Map m)：将m中所有的key-value对存放到当前map中
    Object remove(Object key):移除指定key的key-value对，并返回被删除的value,如果不存在，则返回null
    void clear():清空当前map中的所有数据
         */
        map.put("AA",123);
        map.put(45,123);
        ...
        map.clear(); //并不是把map = null，而是将map中的数据清空
        System.out.println(map.size());
